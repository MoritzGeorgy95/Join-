...
/**
 * All sorts of script variables to quickly access HTML elements from everywhere
 */
...
// handle user closing category option
...
//handle user adding new category
...
//add listeners to every color dot for category selection
for (let i = 0; i < colorDots.length; i++) {
  const dot = colorDots[i];
...
// helper function to scale up dot when it is selected
function scaleUp(e) {
  for (let i = 0; i < colorDots.length; i++) {
    const dot = colorDots[i];
    if (dot.classList.contains("selected")) {
...
//let user select categories & display color dot
...
assignedToInput.addEventListener("click", function () {
  if (contactsDropdownOpen) {
    ...
    for (let i = 0; i < contacts.length; i++) {
      ...
      contactsDropdown.innerHTML += /*html*/ `<div class="contact"><div class="contact-name"><img src="assets/img/icon_name.png"> ${contact.userName}</div> <input type="checkbox" class="checkbox-primary"></div>`;
    }
...
//show all contacts in contacts list
  ...
  for (let i = 0; i < checkboxes.length; i++) {
    ...
    if (checkbox.checked) {
      ...
      document.getElementById("avatarContainer").innerHTML += /*html*/ `
        <div class="chosenContact" style="background: ${contacts[i].color}">${initials}</div>
      `;
    }
  }
}

//hide them when closing the dropdown
...
//add listener to clear task button
